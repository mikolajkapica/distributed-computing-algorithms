...
let handle_computers computers =
  let rec aux computers computers_to_handle =
    match computers_to_handle with
      ...
      | computer :: rest ->
          ...
          | "-1" -> ()
          | _ -> 
              if computer.usage + process.power <= 100 then ( add_process_to_computer computer process; )
              else ask computers computer process;
          aux computers computers_to_handle 
  in
  aux computers computers 

let time_pasage computers =
  let rec aux computers =
    match computers with
    | [] -> ()
    | computer :: rest ->
        computer.usage <- 0;
        computer.current_processes <- [];
        aux rest
  in
  aux computers
